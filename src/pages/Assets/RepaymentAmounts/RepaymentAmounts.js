...
import styles from './RepaymentAmounts.less';
...
@connect(({ assets, loading }) => ({
  assets,
  loading: loading.effects['assets/getRepaymentAmounts'],
}))
class RepaymentAmounts extends PureComponent {
  state = {
    tabActiveKey: 'all',
    params: {
      dayOrWeekOrYear: 1,
  ...
  regularColumns = [
    {
      title: formatMessage({ id: '总放款金额' }),
      dataIndex: 'paymentSum',
      width: 130,
    },
    {
      title: formatMessage({ id: '账单日内已还款金额' }),
    ...
    {
      title: formatMessage({ id: '未还款金额占比' }),
      dataIndex: 'unRepaymentRate',
      width: 145,
    },
    {
      title: formatMessage({ id: '金额还款率（P）' }),
      dataIndex: 'repaymentRateP',
      width: 145,
    },
    {
      title: formatMessage({ id: '金额还款率（T）' }),
      dataIndex: 'repaymentRateT',
    },
  ];

  componentDidMount() {
    this.getRepaymentAmounts();
  }
  ...
  getWidth() {
    ...
    return parseInt(dayOrWeekOrYear, 10) === 1 ? 1875 : 1755;
  }
  ...
  getRepaymentAmounts = () => {
    ...
    dispatch({
      type: 'assets/getRepaymentAmounts',
      payload: params,
    });
  };
    ...
    return (
      ...
      >
        <Card bordered={false}>
          ...
          <Table
            dataSource={repaymentAmounts.list}
            pagination={{
              showSizeChanger: true,
              showQuickJumper: true,
              ...repaymentAmounts.pagination,
            }}
...
export default RepaymentAmounts;
